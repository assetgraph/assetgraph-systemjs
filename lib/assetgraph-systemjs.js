...
module.exports = async function bundleSystemJs(
  assetGraph,
  { polyfill = false, conditions, deferredImports } = {}
) {
  // FIXME: Ideally these would only be registered once,
  // and it would have a nicer syntax
  assetGraph.constructor.JavaScript.registerRelation(
    require('./SystemJsBundle')
  );
  assetGraph.constructor.JavaScript.registerRelation(
    require('./SystemJsLazyBundle')
  );
  ...
  for (const htmlScript of assetGraph.findRelations({ type: 'HtmlScript' })) {
    ...
    if (htmlScript.node.hasAttribute('data-systemjs-csp-production')) {
      ...
      htmlScript.to = assetGraph.addAsset({
        url: assetGraph.resolveUrl(htmlScript.from.nonInlineAncestor.url, href)
      });
